...
const reference = {
  maintainer: "mujtaba.fleapo@gmail.com",
  admin: "thestrives.info@gmail.com",
};
...
const sendNewPassword = (destination) => {
  ...
  const senderEmail = "The Strives <thestrives.info@gmail.com>";
  ...
  return new Promise((res, rej) => {
    pinpoint.sendMessages(params, (err, data) => {
      if (err) rej(err);
      else if (data.MessageResponse.Result[destinationEmail].StatusCode !== 200)
        rej(
          new Error(data.MessageResponse.Result[destinationEmail].StatusMessage)
        );
      else
        res({
          pass: newPassword,
...
const sendTestMail = () => {
  const destinationEmail = reference.maintainer;
  const senderEmail = "The Strives <thestrives.info@gmail.com>";
...
module.exports = { sendNewPassword, sendTestMail };
